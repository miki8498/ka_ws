...
from frankx_ros.msg import PlanPoseArrayTrajectoryAction, PlanPoseArrayTrajectoryActionGoal

from geometry_msgs.msg import Pose, Point, Quaternion, PoseArray
import copy

class MoveitPlanMove(EventState):
    def __init__(self, topic="plan_posearray_trajectory_action", group_name="panda_arm_1"):
        super(MoveitPlanMove, self).__init__(outcomes=["done", "failed"], input_keys=["waypoints"], output_keys=["trajectory_points"])
        self._group_name = group_name
        self._topic = topic
        self._client = ProxyActionClient({self._topic: PlanPoseArrayTrajectoryAction})
    ...
    def on_enter(self, userdata):
        ...
        posearray = PoseArray()
        posearray.poses = [self.list_to_pose(w) for w in userdata.waypoints]


        ################
        # PLANNER
        ################
        msg_plan = PlanPoseArrayTrajectoryActionGoal()
        msg_plan.goal.group_name.data = self._group_name
        msg_plan.goal.goal = posearray
